...
export class LogListComponent implements OnInit {
   ...
   */
  fetchLogs(): void {
    this.chatService.fetchLogs(this.startTime, this.endTime).subscribe(
      (response: any) => {
        ...
        this.toasterService.success({
          detail: 'SUCCESS',
          summary: 'Logs retrieve successfully!',
          duration: 5000,
        });
      },
      (error) => {
        if (error.status === 404) {
          this.toasterService.warning({
            detail: 'WARN',
            summary: 'No log found in this time',
            duration: 5000,
  ...
  onTimeframeChange(): void {
    if (
      ...
      this.startTime = new Date(
        now.getTime() - +this.selectedTimeframe * 60 * 1000
      );
    }
    this.fetchLogs();
  }
